...
Renderer::Renderer(uint T_, ushort _Kalpha, ushort _Kbeta, ushort _Kepsilon,
    ...
    : d_ones(NPTS, 1.f), T(T_), Kalpha(_Kalpha), Kbeta(_Kbeta), Kepsilon(_Kepsilon),
      use_identity(use_identity_), use_texture(use_texture_), use_expression(use_expression_)
{
    HANDLE_ERROR( cudaMalloc( (void**)&ALL_VARS, sizeof(float)*NPTS*600) );
    X0_mean = ALL_VARS;
    Y0_mean = X0_mean + NPTS;
    Z0_mean = Y0_mean + NPTS;
    ...
    HANDLE_ERROR( cudaMalloc( (void**)&d_triangle_idx, sizeof(ushort)*Nredundant) );


    HANDLE_ERROR(cudaMalloc((void**)&d_xp, sizeof(float)*NPTS));
    HANDLE_ERROR(cudaMalloc((void**)&d_yp, sizeof(float)*NPTS));

    HANDLE_ERROR(cudaMalloc((void**)&d_alphas_redundant, sizeof(float)*Nredundant));
    HANDLE_ERROR(cudaMalloc((void**)&d_betas_redundant, sizeof(float)*Nredundant));
    HANDLE_ERROR(cudaMalloc((void**)&d_gammas_redundant, sizeof(float)*Nredundant));

    cudaMemset(d_alphas_redundant, 0, sizeof(float)*Nredundant);
    cudaMemset(d_betas_redundant, 0, sizeof(float)*Nredundant);
    cudaMemset(d_gammas_redundant, 0, sizeof(float)*Nredundant);

    HANDLE_ERROR(cudaMalloc((void**)&d_rend_flag, sizeof(bool)*Nredundant));
    HANDLE_ERROR(cudaMalloc((void**)&d_rend_flag_tmp, sizeof(bool)*Nredundant));

    HANDLE_ERROR(cudaMalloc((void**)&d_tl,  sizeof(ushort)*N_TRIANGLES*3));
    HANDLE_ERROR(cudaMalloc((void**)&d_pixel_idx,  sizeof(ushort)*N_TRIANGLES*NTMP));
    HANDLE_ERROR(cudaMalloc((void**)&d_redundant_idx, sizeof(uint)*Nredundant));

    HANDLE_ERROR( cudaMalloc( (void**)&d_Z, sizeof(float)*NPTS) );
    ...
    float *d_RTEX;
     */
    HANDLE_ERROR(cudaMalloc((void**)&d_REX, sizeof(float)*Nredundant*Kepsilon));
    HANDLE_ERROR(cudaMalloc((void**)&d_REY, sizeof(float)*Nredundant*Kepsilon));
    HANDLE_ERROR(cudaMalloc((void**)&d_REZ, sizeof(float)*Nredundant*Kepsilon));



    if (use_texture)
    {
        HANDLE_ERROR(cudaMalloc((void**)&d_RTEX, sizeof(float)*Nredundant*Kbeta));
    }
     ...
     */
    const int size_I = NPTS*Kalpha;
    const int size_TEX = NPTS*Kbeta;
    ...
    if (h_X0 == NULL)
    {
        vector< vector<float> > X0_vec = read2DVectorFromFile<float>("models/dat_files/X0_mean.dat", NPTS, 1);
        vector< vector<float> > Y0_vec = read2DVectorFromFile<float>("models/dat_files/Y0_mean.dat", NPTS, 1);
        vector< vector<float> > Z0_vec = read2DVectorFromFile<float>("models/dat_files/Z0_mean.dat", NPTS, 1);

        /**
         * Those are the 3D points of the face mesh (of a specific
         * person -- i.e. it's not the average face) before view transformation
         */
        float *X0 = vec2arr(X0_vec, NPTS, 1, false);
        float *Y0 = vec2arr(Y0_vec, NPTS, 1, false);
        float *Z0 = vec2arr(Z0_vec, NPTS, 1, false);

        HANDLE_ERROR( cudaMemcpy( X0_mean, X0, sizeof(float)*NPTS , cudaMemcpyHostToDevice ) );
        HANDLE_ERROR( cudaMemcpy( Y0_mean, Y0, sizeof(float)*NPTS , cudaMemcpyHostToDevice ) );
        HANDLE_ERROR( cudaMemcpy( Z0_mean, Z0, sizeof(float)*NPTS , cudaMemcpyHostToDevice ) );
    ...
    {
        HANDLE_ERROR( cudaMemcpy( X0_mean, h_X0, sizeof(float)*NPTS , cudaMemcpyHostToDevice ) );
        HANDLE_ERROR( cudaMemcpy( Y0_mean, h_Y0, sizeof(float)*NPTS , cudaMemcpyHostToDevice ) );
        HANDLE_ERROR( cudaMemcpy( Z0_mean, h_Z0, sizeof(float)*NPTS , cudaMemcpyHostToDevice ) );
    }
    ...
    ///////////////////////////////////////
    // Mean texture -- BEGIN -- //
    vector< vector<float> > tex_mu_vec = read2DVectorFromFile<float>("models/dat_files/tex_mu.dat", NPTS, 1);
    HANDLE_ERROR( cudaMalloc( (void**)&d_mu_tex, sizeof(float)*NPTS) );

    if (h_tex_mu == NULL)
    {
        float *tex_mu = vec2arr(tex_mu_vec, NPTS, 1, false);

        HANDLE_ERROR( cudaMemcpy( d_mu_tex, tex_mu, sizeof(float)*NPTS , cudaMemcpyHostToDevice ) );
        free(tex_mu);
    }
    else
    {
        HANDLE_ERROR( cudaMemcpy( d_mu_tex, h_tex_mu, sizeof(float)*NPTS , cudaMemcpyHostToDevice ) );
    }
    // Mean texture -- END -- //
    ///////////////////////////////////////
    ///////////////////////////////////////
    ...
    tl_vector = read2DVectorFromFile<int>("models/dat_files/tl.dat", N_TRIANGLES, 3);

    /**
     * Here we store the triangle indices of the face model. I.e., tl is a matrix of
     * size N_TRIANGLESx3 (stored in an array as a column-major matrix), whose each row
     * contains the indices of the face mesh points that form one triangle. Thus,
     * the entries of tl are integers in the range of [0, NPTS]
     */
    ushort *tl;


    tl = (ushort*)malloc( N_TRIANGLES*3*sizeof(ushort) );


    for (uint i=0; i<N_TRIANGLES; ++i) {
        tl[N_TRIANGLES*0+i] = tl_vector[i][0]-1;
        tl[N_TRIANGLES*1+i] = tl_vector[i][1]-1;
        tl[N_TRIANGLES*2+i] = tl_vector[i][2]-1;
    }



    HANDLE_ERROR( cudaMemcpy( d_tl, tl, sizeof(ushort)*N_TRIANGLES*3, cudaMemcpyHostToDevice ) );
    ...
    vector< vector<float> > IX_vec = read2DVectorFromFile<float>("models/dat_files/IX.dat", NPTS, NID_COEFS);
    vector< vector<float> > IY_vec = read2DVectorFromFile<float>("models/dat_files/IY.dat", NPTS, NID_COEFS);
    vector< vector<float> > IZ_vec = read2DVectorFromFile<float>("models/dat_files/IZ.dat", NPTS, NID_COEFS);


    vector< vector<float> > TEX_vec;

    if (use_texture)
        TEX_vec = read2DVectorFromFile<float>("models/dat_files/TEX.dat", NPTS, NTEX_COEFS);
    ...
    for (uint idx=0; idx<NPTS*Kalpha; ++idx) {
        int col = idx % Kalpha;
        int row = idx/Kalpha;
    ...
    if (use_identity)
    {
        HANDLE_ERROR(cudaMallocPitch((void**)&d_IX_row_major, &pitch2, Kalpha * sizeof(float), NPTS));
        HANDLE_ERROR(cudaMallocPitch((void**)&d_IY_row_major, &pitch2, Kalpha * sizeof(float), NPTS));
        HANDLE_ERROR(cudaMallocPitch((void**)&d_IZ_row_major, &pitch2, Kalpha * sizeof(float), NPTS));

        HANDLE_ERROR(cudaMemcpy2D(d_IX_row_major, pitch2, IX_row_major, Kalpha * sizeof(float), Kalpha*sizeof(float), NPTS,  cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMemcpy2D(d_IY_row_major, pitch2, IY_row_major, Kalpha * sizeof(float), Kalpha*sizeof(float), NPTS,  cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMemcpy2D(d_IZ_row_major, pitch2, IZ_row_major, Kalpha * sizeof(float), Kalpha*sizeof(float), NPTS,  cudaMemcpyHostToDevice));
        ...
        HANDLE_ERROR(cudaMalloc((void**)&d_RIX, sizeof(float)*Nredundant*Kalpha));
        HANDLE_ERROR(cudaMalloc((void**)&d_RIY, sizeof(float)*Nredundant*Kalpha));
        HANDLE_ERROR(cudaMalloc((void**)&d_RIZ, sizeof(float)*Nredundant*Kalpha));
    }
    ...
    if (use_texture)
    {
        ...
        HANDLE_ERROR( cudaMallocPitch((void**)&d_TEX_row_major, &pitch3, Kbeta * sizeof(float), NPTS) );
        HANDLE_ERROR( cudaMemcpy2D(d_TEX_row_major, pitch3, TEX_row_major, Kbeta * sizeof(float), Kbeta*sizeof(float), NPTS,  cudaMemcpyHostToDevice) );
    }
    ...
    const int size_E = NPTS*Kepsilon;

    vector< vector<float> > EX_vec = read2DVectorFromFile<float>(config::EX_PATH, NPTS, config::K_EPSILON);
    vector< vector<float> > EY_vec = read2DVectorFromFile<float>(config::EY_PATH, NPTS, config::K_EPSILON);
    vector< vector<float> > EZ_vec = read2DVectorFromFile<float>(config::EZ_PATH, NPTS, config::K_EPSILON);
    ...
    for (uint idx=0; idx<NPTS*Kepsilon; ++idx) {
    ...
    if (use_expression)
    {
         ...
         */
        HANDLE_ERROR(cudaMallocPitch((void**)&d_EX_row_major, &pitch, Kepsilon * sizeof(float), NPTS));
        HANDLE_ERROR(cudaMallocPitch((void**)&d_EY_row_major, &pitch, Kepsilon * sizeof(float), NPTS));
        HANDLE_ERROR(cudaMallocPitch((void**)&d_EZ_row_major, &pitch, Kepsilon * sizeof(float), NPTS));


        HANDLE_ERROR(cudaMemcpy2D(d_EX_row_major, pitch, EX_row_major, Kepsilon * sizeof(float), Kepsilon*sizeof(float), NPTS,  cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMemcpy2D(d_EY_row_major, pitch, EY_row_major, Kepsilon * sizeof(float), Kepsilon*sizeof(float), NPTS,  cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMemcpy2D(d_EZ_row_major, pitch, EZ_row_major, Kepsilon * sizeof(float), Kepsilon*sizeof(float), NPTS,  cudaMemcpyHostToDevice));
...
{
    cudaMemcpy(X0, X0_mean, sizeof(float)*NPTS, cudaMemcpyDeviceToDevice);
    cudaMemcpy(Y0, Y0_mean, sizeof(float)*NPTS, cudaMemcpyDeviceToDevice);
    cudaMemcpy(Z0, Z0_mean, sizeof(float)*NPTS, cudaMemcpyDeviceToDevice);
    ...
    if (use_identity)
    {
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kalpha, &alpha, d_IX, NPTS, ov.alphas, 1, &beta, X0, 1);
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kalpha, &alpha, d_IY, NPTS, ov.alphas, 1, &beta, Y0, 1);
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kalpha, &alpha, d_IZ, NPTS, ov.alphas, 1, &beta, Z0, 1);
    }


    if (use_expression)
    {
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kepsilon, &alpha, d_EX, NPTS, ov.epsilons, 1, &beta, X0, 1);
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kepsilon, &alpha, d_EY, NPTS, ov.epsilons, 1, &beta, Y0, 1);
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kepsilon, &alpha, d_EZ, NPTS, ov.epsilons, 1, &beta, Z0, 1);
    }

    view_transform_3d_pts_and_render_2d<<<(NPTS+NTHREADS-1)/NTHREADS, NTHREADS>>>(X0, Y0, Z0,
    ...
    cudaMemcpy(&xle, d_xp+19106, sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(&yle, d_yp+19106, sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(&xre, d_xp+21516, sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(&yre, d_yp+21516, sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(&xm, d_xp+8235, sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(&ym, d_yp+8235, sizeof(float), cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(d_Z, Z, sizeof(float)*NPTS, cudaMemcpyDeviceToDevice);
...
{
    cudaMemcpy(Xcur, X0_mean, sizeof(float)*NPTS, cudaMemcpyDeviceToDevice);
    cudaMemcpy(Ycur, Y0_mean, sizeof(float)*NPTS, cudaMemcpyDeviceToDevice);
    cudaMemcpy(Zcur, Z0_mean, sizeof(float)*NPTS, cudaMemcpyDeviceToDevice);
    ...
    if (use_identity)
    {
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kalpha, &alpha, d_IX, NPTS, ov.alphas, 1, &beta, Xcur, 1);
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kalpha, &alpha, d_IY, NPTS, ov.alphas, 1, &beta, Ycur, 1);
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kalpha, &alpha, d_IZ, NPTS, ov.alphas, 1, &beta, Zcur, 1);
    }
    ...
    rotate_3d_pts<<<(NPTS+NTHREADS-1)/NTHREADS, NTHREADS>>>(Xcur, Ycur, Zcur, R);
}
...
{
    cudaMemcpy(o.d_tex, d_mu_tex, sizeof(float)*NPTS, cudaMemcpyDeviceToDevice);
    float alpha = 1.0f;
    float beta  = 1.0f;

    if (use_texture) {
        cublasSgemv(handle, CUBLAS_OP_N, NPTS, Kbeta, &alpha, d_TEX, NPTS, ov.betas, 1, &beta, o.d_tex, 1);
    }
...
void Renderer::print_obj(const std::string& obj_path)
{
    ...
    h_X = (float*)malloc( NPTS*sizeof(float) );
    h_Y = (float*)malloc( NPTS*sizeof(float) );
    h_Z = (float*)malloc( NPTS*sizeof(float) );

    cudaMemcpy(h_X, X, sizeof(float)*NPTS, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_Y, Y, sizeof(float)*NPTS, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_Z, Z, sizeof(float)*NPTS, cudaMemcpyDeviceToHost);

    vector< vector<int> > tlv = read2DVectorFromFile<int>("models/dat_files/tl.dat", N_TRIANGLES, 3);
...
void Renderer::print_obj_neutral(const std::string& obj_path)
{
    ...
    cudaMemcpy(h_X, X0, sizeof(float)*NPTS, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_Y, Y0, sizeof(float)*NPTS, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_Z, Z0, sizeof(float)*NPTS, cudaMemcpyDeviceToHost);
...
void Renderer::print_sparse_3Dpts(const std::string& obj_path)
{
    ...
    for (uint i=0; i<NLANDMARKS_51; ++i) {
        ofs <<  h_X[lis[i]] << " " << h_Y[lis[i]] << " " << h_Z[lis[i]] << std::endl;
    }
...
void Renderer::print_sparse_2Dpts(const std::string& obj_path, float _resize_coefl  )
{
    ...
    h_xp = (float*)malloc( NPTS*sizeof(float) );
    h_yp = (float*)malloc( NPTS*sizeof(float) );

    cudaMemcpy(h_xp, d_xp, sizeof(float)*NPTS, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_yp, d_yp, sizeof(float)*NPTS, cudaMemcpyDeviceToHost);
    std::vector<float> xp, yp;

    for (uint i=0; i<NLANDMARKS_51; ++i) {
        xp.push_back(_resize_coefl*h_xp[lis[i]] - config::PAD_SINGLE_IMAGE);
        yp.push_back(_resize_coefl*h_yp[lis[i]] - config::PAD_SINGLE_IMAGE);
    }
    ...
    for (uint i=0; i<NLANDMARKS_51; ++i) {
        ofs <<  _resize_coefl*h_xp[lis[i]] - config::PAD_SINGLE_IMAGE << " " << _resize_coefl*h_yp[lis[i]] - config::PAD_SINGLE_IMAGE  << std::endl;
    }
