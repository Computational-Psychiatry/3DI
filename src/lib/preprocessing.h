...
cv::Rect detectFaceOpenCVDNN_multiscale(cv::dnn::Net net, cv::Mat &frameOpenCVDNN, std::string framework, double *confidence=NULL);
cv::Rect detectFaceOpenCVDNN(cv::dnn::Net net, cv::Mat &frameOpenCVDNN, std::string framework, double *confidence=NULL, cv::Rect *ROI=NULL);
cv::Point transform_pt(float px, float py, const cv::Point2f& center, float scale, float resolution, bool invert = false);

double st_dev(std::vector<float> v, float ave);


//void detect_landmarks_opencv(cv::dnn::Net& detection_net, cv::dnn::Net &landmark_net,
//                      const std::string& framework, cv::Mat &frame,
//                      std::vector<float>& xp_vec, std::vector<float>& yp_vec, float &bbox_size);

void detect_landmarks_opencv(const cv::Rect& d, double face_confidence, cv::dnn::Net& landmark_net, cv::dnn::Net& leye_net, cv::dnn::Net& reye_net, cv::dnn::Net& mouth_net, cv::dnn::Net &correction_net, cv::Mat &frame,
                             std::vector<float>& xp_vec, std::vector<float>& yp_vec, float &bbox_size,
                             std::vector<float> &xrange, std::vector<float> &yrange, bool use_local_models, bool plot=false,
                             std::vector<std::vector<double> > *xs = NULL, std::vector<std::vector<double> > *ys = NULL);

void detect_landmarks_opencv_single(const cv::Rect& d, double face_confidence, cv::dnn::Net& landmark_net, cv::dnn::Net& leye_net, cv::dnn::Net& reye_net, cv::dnn::Net& mouth_net, cv::dnn::Net &correction_net, cv::Mat &frame,
                             std::vector<float>& xp_vec, std::vector<float>& yp_vec, float &bbox_size,
                             std::vector<float> &xrange, std::vector<float> &yrange, bool use_local_models, bool plot=false);


cv::Rect detect_face_opencv(cv::dnn::Net& detection_net, const std::string& framework, cv::Mat &frame, cv::Rect *prev_d=NULL, double* face_confidence=NULL, bool multiscale_start=false);
...
std::pair<std::vector<float>, std::vector<float>> compute_landmarks_wlocal_models(cv::Mat& frame, int face_size,
                                                                                  ...
                                                                                  cv::dnn::Net& leye_net, cv::dnn::Net& reye_net, cv::dnn::Net& mouth_net, double tx_rate, double ty_rate);

void paint_innermouth_black(cv::Mat& frame, const std::vector<float>& xp_vec, const std::vector<float>& yp_vec);

cv::Point2f transform_pt(float px, float py, const cv::Point2f* center, float* scale, float* resolution, bool invert = false);
